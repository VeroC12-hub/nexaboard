...
export default function DrawingModal({ onInsert, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl flex flex-col mx-4" style={{ width: 700, maxHeight: '90vh' }}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-green-100 shrink-0">
          <h2 className="text-base font-bold text-[#1b2b4b]">Draw &amp; Shapes</h2>
          <button onClick={onClose} className="text-[#9ca3af] hover:text-[#1b2b4b] transition-colors"><X size={18} /></button>
        </div>

        {/* Toolbar */}
        <div className="flex items-center gap-2 px-4 py-2 border-b border-green-100 shrink-0 flex-wrap bg-[#fafffe]">
          {/* Shape tools */}
          <div className="flex items-center bg-white border border-green-200 rounded-lg p-0.5 gap-0.5">
          ...
          <div className="flex items-center gap-1.5">
            {COLORS.map(c => (
              <button key={c} onClick={() => setColor(c)}
                className={`w-6 h-6 rounded-full border-2 transition-all hover:scale-110 ${color === c ? 'border-[#1b2b4b] scale-110' : 'border-gray-200'}`}
          ...
          {fillable && (
            <>
              ...
              <button onClick={() => setFill(v => !v)}
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors ${fill ? 'bg-[#5ab82e] text-white border-[#5ab82e]' : 'border-green-200 text-[#6b7280] hover:border-[#5ab82e]'}`}>
                Fill
              </button>
            </>
          )}
          ...
          <div className="ml-auto flex gap-1.5">
            <button onClick={undo} title="Undo last shape"
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-[#6b7280] hover:text-[#1b2b4b] border border-green-200 rounded-lg transition-colors">
              <Undo2 size={12} /> Undo
            </button>
            <button onClick={clear}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-red-500 hover:bg-red-50 border border-red-100 rounded-lg transition-colors">
        ...
        {/* Canvas */}
        <div className="flex-1 overflow-auto p-4 bg-[#f8fffe] flex items-center justify-center">
          <canvas
            ref={canvasRef}
            width={620}
            height={360}
        ...
        {/* Footer */}
        <div className="flex justify-end gap-2 px-5 py-3.5 border-t border-green-100 shrink-0">
